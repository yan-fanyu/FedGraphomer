...
class MethodGraphBertNodeClassification(BertPreTrainedModel):
    ...
    def train_model(self, max_epoch):
        ...
        _train = self.data['idx_train']
        _val = self.data['idx_val']
        _test = self.data['idx_test']
        ...
        for epoch in range(max_epoch):
            ...
            accuracy.data = {'true_y': self.data['y'][self.data['idx_val']], 'pred_y': output.max(1)[1]}
        ...
        loss = F.cross_entropy(output, self.data['y'][self.data['idx_test']])
        accuracy.data = {'true_y': self.data['y'][self.data['idx_test']], 'pred_y': output.max(1)[1]}
        acc_test = accuracy.evaluate()
        acc = acc_test.item()

        print("loss = {:.4f} \taverage test accuracy: {:.4f}".format(loss, acc))
        return time.time() - t_begin, acc
